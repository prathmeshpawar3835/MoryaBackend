...
public class ConcurrencyTests
{
    ...
    public async Task Concurrent_bills_receive_distinct_numbers()
    {
        ...
        async Task<string> CreateBillAsync()
        {
            ...
            var billing = new BillingService(db, user, new StockEngine(db), new DocumentNumberGenerator(db), new AuditService(db, user), new ReferralService(db, user, new AuditService(db, user)), new ReturnDocumentService(db, user, new StockEngine(db), new DocumentNumberGenerator(db), new ReferralService(db, user, new AuditService(db, user))), new BirthdayService(db, user, new DisabledWhatsAppService(), new AuditService(db, user)));
